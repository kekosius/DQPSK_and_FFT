 ...
 * одинаковых кодов больше единицы (Counter > 1)
 */
 ...
 *
 * @param	   High Последовательность исходного кода (старшая тетрада)
 * @param	   Low Последовательнось исходного кода (младшая тетрада)
 *
 * @retval     BadState Идентификатор ошибки декодирования
 *
 * Выполняет поиск АРСФ кода для исходной последовательности. Если код не был
 * обнаружен, массив сдвигается вправо (см. #ArrayShift), поиск продолжается
 * вплоть до 7-ми сдвигов. Если был получен достоверный код (т.е. код был обнаружен
 * в последовательности более одного раза), то результат отправляется по USART и
 * выводится на LCD экран (см. #LCD_DQPSK_Result), программа возвращает 0. Если
 * достоверный код не был обнаружен, то по USART отправляется сообщение об ошибке, 
 * программа возвращает 1. При долгом отсутсвии приёма (более 1,5 сек) информация
 * о коде пропадёт и с LCD экрана (см. #LCD_Show_No_Result).
 */
...
/*!
 * @brief      Табличный переход от кода Бауера к исходному числу
 *
 * @param	   data демодулированное hex число (код Бауера)
 * @param	   ErrorVar ссылка на идентификатор ошибки перехода
 *
 * @retval     Orig исходный код
 *
 * Возвращает табличное значение исходного кода, которому соответсвует полученный
 * код Бауера. Если такого кода нет, то возвращает невозможное число 0xff, 
 * *ErrorVar = 1
 */
...
/*!
 * @brief      Табличный переход от hex кода к разрешенной скорости
 *
 * @param	   DQPSK_code декодированный АРСФ код
 *
 * @retval     speedCode Разрешенные скорости
 *
 * Возвращает табличное значение разрешенной скорости, которая соответсвует 
 * полученному АРСФ коду. Скорость возвращается в формате XXYY, где XX - КРЦ,
 * YY - СРЦ. Информация о разрешенной скорости используется для отображения на
 * LCD экране (см. #LCD_DQPSK_Result)
 */
