...
//#define MOVE_AVG        //Разкомментировать для использования скользящей амплитуды

#define SEND_AVG		/*!< Если SEND_AVG define, то система отправляет значение напряжения
						 *   #AverageVoltage по USART */
 ...
 * @retval    СКЗ средняя амплитуда сигнала в милливольтах
 *
 * Рассчитывает среднеквадратичное значение амплитуды (аналогично мультиметру). 
 * СКЗ = Ап*0,707 (Ап - половина размаха т.е. полной амплитуды)
 *
 ...
 *
 * @param	  avg Текущее значение амплитуды
 * @retval    movingAvg Значение амплитуды с учётом предыдущих измерений
 */
